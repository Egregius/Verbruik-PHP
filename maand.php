...
echo '
		<div style="width:920px">
			...
			foreach($selectedusers as $s)echo '<input type="hidden" name="'.$s.'" value="on"/>';
			echo '<select name="f_maand" class="btn b4 btna" onchange="this.form.submit()"/>';
			$months=array(3,6,9,12,24,36,48,60,72,84,96,108,120);
			foreach($months as $k){
				if($k==$_SESSION['f_maand'])echo '<option value="'.$k.'" selected>'.$k.' maanden</option>';
				else echo '<option value="'.$k.'">'.$k.' maanden</option>';
			}
			echo '
			</select>
			</form>
...
foreach($selectedusers as $s){
	...
	$tabel=$s.'dag';
	for($k=0;$k<=119;$k++){
		$mindate=strtotime(' -1 month',$maxdate);
...
foreach($selectedusers as $s){
	...
	foreach($items as $item){
		...
		if(isset($item[$s.'gas']))$graphgas[$index][$s.' gemiddelde']=number_format($totalgas/$i,3);else $graphgas[$index][$s.' gemiddelde']=0;
		...
		if(isset($item[$s.'elec']))$graphelec[$index][$s.' gemiddelde']=number_format($totalelec/$i,3);else $graphelec[$index][$s.' gemiddelde']=0;
		if($users[$s]['zon']==1){
		...
		if(isset($item[$s.'verbruik']))$graphverbr[$index][$s.' gemiddelde']=number_format($totalverbruik/$i,3);else $graphverbr[$index][$s.' gemiddelde']=0;
...
$args=array('width'=>900,
			...
			'x_axis_gridlines'=>8,
			'y_axis_gridlines'=>6,
			'raw_options'=>'lineWidth:4,
							crosshair:{trigger:"both"},
							series:{
								0:{lineDashStyle:[0,0],pointSize:7},
								1:{lineDashStyle:[4,4],curveType:"function"},
								2:{lineDashStyle:[0,0],pointSize:7},
								3:{lineDashStyle:[4,4],curveType:"function"},
								4:{lineDashStyle:[0,0],pointSize:7},
								5:{lineDashStyle:[4,4],curveType:"function"},
								6:{lineDashStyle:[0,0],pointSize:7},
								7:{lineDashStyle:[4,4],curveType:"function"},
								8:{lineDashStyle:[0,0],pointSize:7},
								9:{lineDashStyle:[4,4],curveType:"function"},
							},vAxis: {
...
if($numberzonusers>0){
	...
	foreach($selectedusers as $s)echo'<td>'.number_format(@$i[$s.'water'],3,',','.').'</td>';
if($_SESSION['euro']=='true'){
	foreach($selectedusers as $s)echo'<td>'.number_format(@$i[$s.'gas']+@$i[$s.'verbruik']+@$i[$s.'water'],2,',','.').'</td>';
}
	echo '</tr>';
}
